...
3. Dentro de init.el, en el apartado de ~package-selected-packages~ se encuentra la lista de paquetes que se han instalado desde ~M-x package-install RET~ (o sea, dentro de emacs). Instalarlos de esa manera los almacena dentro de la carpeta ~elpa~ que se encuentra casi siempre en ~~/.emacs.d/~
